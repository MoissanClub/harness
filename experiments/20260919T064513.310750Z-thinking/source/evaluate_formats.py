    ...
    for cache in ("cold", "warm"):
        ...
        for format_name in FORMATS:
            ...
            result[cache][format_name] = {
                ...
                "metrics": metrics,
                "successful_plan_median_s": statistics.median(
                    row["total_request_seconds"] for row in group if successful(row)
                ) if any(successful(row) for row in group) else None,
            }
...
def main():
    ...
    final_props = request_json(args.base_url.rstrip("/")[:-3] + "/props")
    template_same = final_props["chat_template"] == props["chat_template"]
    save(output / "verification.json", {"template_unchanged": template_same})
    print(f"Saved experiment: {output}", flush=True)
    return int(any(row.get("error") for row in rows) or not template_same)
