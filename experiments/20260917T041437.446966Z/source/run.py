...
def evaluate_response(response, elapsed, format_name, max_actions):
    ...
    summary["only_tool_output"] = summary["protocol_ok"] and not content.strip() and not reasoning.strip()
...
def main() -> int:
    ...
    parser.add_argument("--format", choices=FORMATS, default="combined")
    ...
    for repetition in range(args.repeat):
        for case_name, case in selected.items():
            ...
            try:
                ...
                exit_code |= int(not summary["only_tool_output"] or not
