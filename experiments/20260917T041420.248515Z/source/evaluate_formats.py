...
from run import ROOT, add_common_arguments, build_payload, request_json, run, save
...
def inspect_encodings(args, output):
    ...
    for format_name in FORMATS:
        ...
        for name, actions in {"one": ["forward"], "two": ["turn_left", "forward"],
            ...
            example["messages"].append({"role": "assistant", "content": "",
                                        "tool_calls": encode_actions(actions, format_name)})
    ...
    for cache in ("cold", "warm"):
        ...
        for format_name in FORMATS:
            ...
            result[cache][format_name] = {
                ...
                "successes": sum(row.get("assessment", {}).get("qualitative_goal_match", False) for row in group),
                "scenes": {case: {
                    "n": sum(row["case"] == case for row in group),
                    "successes": sum(row["case"] == case and row.get("assessment", {}).get("qualitative_goal_match", False)
                                     for row in group),
...
def main():
    ...
    sources = ["run.py", "tools.py", "evaluate_formats.py", "request-defaults.json", "cases.json",
               "FORMAT_HYPOTHESIS.md", "prompts/system.txt", "prompts/user.txt"]
    ...
    save(output / "manifest.json", {
        ...
        "scheduler_seed": 20260917, "sampling": {"temperature": 1.0, "top_k": 64, "top_p": 0.95, "min_p": 0.0},
        "design": "Each randomly ordered format: cold request then identical warm request; one model response each.",
    })
    ...
    for index, item in enumerate(schedule, 1):
        ...
        try:
            row = run(local, run_dir, item["case"], cases[item["case"]], props=props, quiet=True,
                      overrides={"temperature": 1.0, "top_k": 64, "top_p": 0.95, "min_p": 0.0})
